...
public class WishAdapter extends BaseAdapter {
	...
	public View getView(int arg0, View arg1, ViewGroup arg2) {
		...
		try {
			Picasso.with(context).load(Utils.URL + list.get(arg0).get("user_id")).into(item.userImage);
		...
		public void onClick(View arg0) {
			...
			if (arg0.getId() == R.id.achieve) {
				if (list.get(pos).get("user_id").toString()
					...
					Toast.makeText(context, "这是你自己的心愿单喔", Toast.LENGTH_SHORT).show();
				} else if (Utils.user_id.equals("")) {
					Intent intent3 = new Intent(context, LoginActivity.class);
					context.startActivity(intent3);
				} else {
					HelpAchievePopWindow helpAchievePop = new HelpAchievePopWindow(context,
							list.get(pos));
