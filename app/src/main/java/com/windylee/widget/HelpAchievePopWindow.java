...
public class HelpAchievePopWindow extends PopupWindow implements OnClickListener {
    ...
    public HelpAchievePopWindow(Context context, Map<String, Object> map) {
        super(context);
    ...
    public void onClick(View arg0) {
        ...
        switch (arg0.getId()) {
            ...
            case R.id.hc_pop_tell_txv:
                OldBookFactory.getCenterSingleton().setWishStatus(Utils.user_id, map.get("wish_id").toString(), 2, Utils.username)
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribe(new Action1<String>() {
                            @Override
                            public void call(String result) {
                                if (result.equals("success")) {
                                    Toast.makeText(context, "已成功通知对方，耐心等待他联系你吧", Toast.LENGTH_SHORT).show();
                                    dismiss();
                                } else {
                                    Toast.makeText(context, "好像出错了呢，再试一次吧", Toast.LENGTH_SHORT).show();
                                }
                            }
                        });
                break;
