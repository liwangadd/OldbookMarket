...
import com.windylee.activity.AddBookActivity;
import com.windylee.activity.BookDetailActivity;
import com.windylee.activity.BookListActivity;
import com.windylee.activity.BookMarketActivity;
import com.windylee.activity.LoginActivity;
import com.windylee.activity.MainActivity;
import com.windylee.activity.SearchActivity;
...
//        }
        ...
        UserManageService uService = OldBookFactory.getManageSingleton();
        uService.getMessage("").observeOn(AndroidSchedulers.mainThread())
        ...
        uService.hasMessage().observeOn(AndroidSchedulers.mainThread())
    ...
    public void resetService() {
        bookService.getHomePageInfo(Utils.curUniversity).observeOn(AndroidSchedulers.mainThread())
                ...
                .subscribe(new Action1<Map<String, Object>>() {
                    @Override
                    public void call(Map<String, Object> map) {
                        newlist = (List<Map<String, Object>>) map.get("books1");
                        if (newlist.size() != 0) {
                            int size = 0;
                            if (newlist.size() < 2) {
                                size = newlist.size();
                            ...
                            for (int i = 0; i < size; i++) {
                                ...
                                Picasso.with(getActivity()).load(Utils.IMGURL + newlist.get(i).get("imgs").toString().substring(1, 37)).into(mainPageImage[i]);
...
//										GetImgeLoadOption.getBookOption());
                            ...
                            Toast.makeText(getActivity(), "该校区未获取到数据", Toast.LENGTH_SHORT)
                                    .show();
                        }

                        wholist = (List<Map<String, Object>>) map.get("books2");
                        if (wholist.size() != 0) {
                            int size = 0;
                            if (wholist.size() < 2) {
                                size = wholist.size();
                            ...
                            for (int i = 0; i < size; i++) {
                                ...
                                mainPageSummary[2 + i].setText(wholist.get(i)
                                        .get("description").toString());
                                Picasso.with(getActivity()).load(Utils.IMGURL + newlist.get(i).get("imgs").toString().substring(1, 37)).into(mainPageImage[2 + i]);
//								mImageLoader.displayImage(Utils.IMGURL
        ...
        switch (v.getId()) {
            ...
            case R.id.newbook1:
                ...
                if (newlist.size() > 0) {
                    ...
                    Toast.makeText(getActivity(), "该书不存在", Toast.LENGTH_SHORT).show();
                }
                break;
            case R.id.newbook2:
                intent = new Intent(getActivity(), BookDetailActivity.class);
